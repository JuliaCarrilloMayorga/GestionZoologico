...
public class CRUDTratamientos extends CRUDPadre {
	...
	private void configurarNombres() {
		nombreClase = "Tratamiento";
		nombreClasePlur = "Tratamientos";
		campos = new String[] {"id", "Descripción", "Coste"};
		masculino=true;
	}
	
	//CONSTRUCTOR
	public CRUDTratamientos(JFrame padre, boolean modal, Persistencia per2) {
		super(padre, modal);
	...
	private void eliminar() {
		...
		Set<Animaltratamiento> aniTra = tratamiento.getAnimaltratamientos(per);
		Set<Empleado> empl = tratamiento.getEmpleados(per);

		if(empl.size()>0) {
			JOptionPane.showMessageDialog(this, "No puede eliminar el tratamiento con id " + id + ", ya que tiene "+ empl.size() +" empleado"+ (empl.size()==1?"":"s")+" relacionado"+(empl.size()==1?"":"s")+".", "Tratamiento no eliminado", JOptionPane.WARNING_MESSAGE);

		} else if(aniTra.size()>0) {
			JOptionPane.showMessageDialog(this, "No puede eliminar el tratamiento con id " + id + ", ya que tiene "+ aniTra.size() +" tratamiento"+ (aniTra.size()==1?"":"s")+" de animales relacionado"+(aniTra.size()==1?"":"s")+".", "Tratamiento no eliminado", JOptionPane.WARNING_MESSAGE);

		} else {
			try {
				int res = JOptionPane.showConfirmDialog(this, "Está seguro de que quiere eliminar el tratamiento de id: "+id+"?", "Borrar tratamiento", JOptionPane.YES_NO_OPTION);

				if(res==JOptionPane.YES_OPTION) {
					per.eliminarObjeto("Tratamiento",tratamiento);
					JOptionPane.showMessageDialog(this, "Se ha eliminado el tratamiento de id " + id + ".", "Tratamiento eliminado", JOptionPane.INFORMATION_MESSAGE);
				}
				reiniciar();
	...
	protected void configurar(ElementoPadre elementoPadre) {
		Tratamiento ali = (Tratamiento) elementoPadre;
	...
	protected boolean validarCampos(boolean nuevo) {
		...
		if(descripcion.equals("") || coste.equals("")) { // Comprobar descripcion con contenido
			JOptionPane.showMessageDialog(this, "Debe completar el campo de DESCRIPCIÓN y COSTE.", "Campo incompleto", JOptionPane.WARNING_MESSAGE);
			return false;
		}
		
		if(!coste.matches("[0-9]+(\\.[0-9]+)?") || Double.valueOf(coste)==0) {
			JOptionPane.showMessageDialog(this, "Debe completar el campo de COSTE con un número válido mayor que 0.", "Campo incorrecto", JOptionPane.WARNING_MESSAGE);
			return false;
		} 

		try {
			Tratamiento ali = (Tratamiento)per.getObjetoMulti("Tratamiento", new String[] {"descripcion", descripcion});
			
			if(nuevo) { //Si nuevo:
				if(ali!=null) { //La zona no debe existir
					JOptionPane.showMessageDialog(this, "Ya existe un tratamiento con descripcion " + descripcion +".", "Tratamiento ya existente", JOptionPane.WARNING_MESSAGE);
					return false;
				} 
				...
				if(ali!=null && (ali.getId()!=id)) { //La zona no debe coincidir con otro distinto al modificado
					JOptionPane.showMessageDialog(this, "Ya existe un tratamiento distinto (ID: "+ ali.getId() +") con descripcion " + descripcion, "Tratamiento ya existente", JOptionPane.WARNING_MESSAGE);
					return false;
				}
			}
			return true;
	...
	private Vector resultadosBusqueda(String descripcion) {
		...
		try {
			ArrayList<Tratamiento> tratamiento = (ArrayList<Tratamiento>) per.getObetosFiltradoMulti("Tratamiento", new String[]{"descripcion", descripcion}, new String[]{});
			
			for (Tratamiento a : tratamiento) {
				Vector o = new Vector();
				o.add(a.getId());
	...
	protected void prepararModificar(String[] parametros) {
		...
		try {
			tratamiento = (Tratamiento)per.getObjetoMulti("Tratamiento", new String[] {"descripcion", descripcion});
			
			textFieldId.setText(String.valueOf(tratamiento.getId()));
			textFieldDescripcion.setText(tratamiento.getDescripcion());
			textFieldCoste.setText(String.valueOf(tratamiento.getCoste()));
