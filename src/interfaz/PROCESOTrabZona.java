...
import tablas.ElementoPadre;
import tablas.Empleado;
import tablas.Tratamiento;
import tablas.Zona;
import utilidades.Utilidades;

public class PROCESOTrabZona  extends PROCESOPadre {
	...
	private void configurar() {
		nombreProceso="Zonas de los Trabajadores";
	...
	public PROCESOTrabZona(JFrame padre, boolean modal, Persistencia per) {
		...
		JLabel lblId_3 = new JLabel("Zona:");
		lblId_3.setBounds(10, 15, 67, 24);
		panelUnico.add(lblId_3);
		
		comboBoxZona = new JComboBox();
		
		comboBoxZona.setBounds(87, 15, 284, 20);
		panelUnico.add(comboBoxZona);
	...
	protected void guardarCambios() {
		...
		if(guardado) {
			mensaje="Va a guardar la relación de "+ zona.getDescripcion() +" y " + empleado.getNombre() + ".\n"
					+ "\n¿Está de acuerdo?" ;
		} else if(elemEliminados>0) {
			mensaje="Va a eliminar "+elemEliminados+ " relacion" + (elemEliminados==1?"":"es") + " de "+ zona.getDescripcion() +".\n"
					+ "\n¿Está de acuerdo?" ;
		}
		...
		if(res==JOptionPane.YES_OPTION) {
			try {
				per.guardarObjeto("Zona", zona); //Guarda el objeto de inverse true (Hibernate) o hace commit (JDBC)
	...
	protected void eliminarProceso() {
		...
		try {
			...
			per.borrarProceso("trabaja", new ElementoPadre[] {zona, empleado}, new String[] {"idZona", String.valueOf(zona.getId()), "idEmpleado", String.valueOf(empleado.getId())});
	...
	protected void aniadirProceso() {
		try {
			per.aniadirProceso("trabaja", new ElementoPadre[] {zona, empleado}, new String[] {String.valueOf(zona.getId()), String.valueOf(empleado.getId())});
	...
	protected Vector objetosTabla(){
		...
		if (zona != null) { //si se ha seleccionado zona, activa el comboBox de empleado
			comboBoxEmpleado.setEnabled(true);
			if(empleado==null) { //Si NO se ha seleccionado empleado, muestra todos los de esa zona
				empleados = zona.getEmpleados(per);
			} else {  //Si se ha seleccionado y están relacionados, muestra la relación, si no activa el Añadir
				if(zona.getEmpleados(per).contains(empleado)) {
	...
	private void configurarElementos(){
		...
		try {
			if(zonaS.equals("")) {
				...
				zona = (Zona)per.getObjetoMulti("Zona", new String[] {"descripcion", zonaS});
			}
