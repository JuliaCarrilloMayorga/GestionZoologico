...
public class CRUDEspecies extends CRUDPadre {
	...
	private void configurarNombres() {
		nombreClase = "Especie";
		nombreClasePlur = "Especies";
		campos = new String[] {"id", "Descripción"};
		masculino=false;
	}
	
	//CONSTRUCTOR
	public CRUDEspecies(JFrame padre, boolean modal, Persistencia per2) {
		super(padre, modal);
	...
	private void eliminar() {
		...
		per.actualizarSesion(especie);

		Set<Animal> animales = especie.getAnimals(per);

		if(animales.size()>0) {
			JOptionPane.showMessageDialog(this, "No puede eliminar la especie con id " + id + ", ya que tiene "+ animales.size() +" animal"+(animales.size()==1?"":"es")+" relacionado"+(animales.size()==1?"":"s")+".", "Especie no eliminada", JOptionPane.WARNING_MESSAGE);

		} else {
			try {
				int res = JOptionPane.showConfirmDialog(this, "Está seguro de que quiere eliminar la especie de id: "+id+"?", "Borrar especie", JOptionPane.YES_NO_OPTION);

				if(res==JOptionPane.YES_OPTION) {
					per.eliminarObjeto("Especie",especie);
					JOptionPane.showMessageDialog(this, "Se ha eliminado la especie de id " + id + ".", "Especie eliminada", JOptionPane.INFORMATION_MESSAGE);
				}
				reiniciar();
	...
	protected void configurar(ElementoPadre elementoPadre) {
		Especie especie = (Especie) elementoPadre;
		especie.setDescripcion(textFieldDescripcion.getText().trim().toUpperCase());
	}
	...
	protected boolean validarCampos(boolean nuevo) {
		...
		try {
			Especie e = (Especie)per.getObjetoMulti("Especie", new String[] {"descripcion", descripcion});
			if(nuevo) { //Si nuevo:
				if(e!=null) { //La especie no debe existir
					JOptionPane.showMessageDialog(this, "Ya existe una especie con descripcion " + descripcion +".", "Especie ya existente", JOptionPane.WARNING_MESSAGE);
					return false;
				}
			} else { // Si es modificación:
				int id = Integer.valueOf(textFieldId.getText());
				
				if(e!=null && (e.getId()!=id)) { //La especie no debe coincidir con otra distinto al modificado
					JOptionPane.showMessageDialog(this, "Ya existe una especie distinto (ID: "+ e.getId() +") con descripción " + descripcion, "Especie ya existente", JOptionPane.WARNING_MESSAGE);
					return false;
				}
			}
			return true;
	...
	protected void buscar() {
		...
		CRUDTablaGeneral ventanaEspecies = new CRUDTablaGeneral(this, null, true, resultados, parametros);
	...
	private Vector resultadosBusqueda(String descripcion) {
		//{"id", "Descripcion"};
		Vector especiesV= new Vector();
		try {
			ArrayList<Especie> especies = (ArrayList<Especie>) per.getObetosFiltradoMulti("Especie", new String[]{"descripcion", descripcion}, new String[]{});
			
			for (Especie a : especies) {
				Vector especie = new Vector();
				especie.add(a.getId());
				especie.add(a.getDescripcion());
				
				especiesV.add(especie);
			}
	...
	protected void prepararModificar(String[] parametros) {
		...
		try {
			especie = (Especie)per.getObjetoMulti("Especie", new String[]{"descripcion", descripcion});
			textFieldId.setText(String.valueOf(especie.getId()));
			textFieldDescripcion.setText(especie.getDescripcion());
